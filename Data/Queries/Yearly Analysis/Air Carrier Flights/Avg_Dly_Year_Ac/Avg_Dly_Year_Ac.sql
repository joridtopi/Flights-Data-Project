...
CREATE TABLE Avg_Dly_Year_Ac AS
...
ALTER TABLE Avg_Dly_Year_Ac
ALTER COLUMN op_unique_carrier TYPE varchar(20);

INSERT INTO Avg_Dly_Year_Ac
SELECT EXTRACT(YEAR from fl_date) AS fl_year,
		avg(dep_delay) AS Avg_Dep_Dly_Year,
		avg(arr_delay) AS Avg_Arr_Dly_Year,
		avg(carrier_delay) AS Avg_Carrier_Dly_Year,
		...
		avg(nas_delay) AS Avg_Nas_Dly_Year,
		avg(security_delay) AS Avg_Security_Dly_Year,
		avg(weather_delay) AS Avg_Weather_Dly_Year, op_unique_carrier
FROM flights_2018
GROUP BY fl_year, cancelled, op_unique_carrier
HAVING cancelled = '0';
